...
def cleanFile():
    #read the file and remove empty spaces from fields
     ...
     CoaaplicantIncome_e = pd.Series(LabelEncoder().fit_transform(dataFile["CoapplicantIncome"]))
     ...
     e=pd.concat([gender_e,married_e,Dependents_e,education_e,Self_Employed_e,Loan_Amount_Term_e,Credit_History_e,Property_Area_e, Loan_Status_e], axis=1)     
     e.columns = ['Gender','Married','Dependents','Education','Self_Employed','ApplicantIncome','CoapplicantIncome','Loan_Amount_Term','LoanAmount','Credit_History','Property_Area','Loan_Status']
     return e
